import date_utils from "../js/date_utils.js";
import { $, createSVG, animateSVG } from "../js/svg_utils.js";
...
export default class Bar {
  ...
  update_bar_position({ x = null, width = null }) {
    ...
    this.draw_bar();
    this.draw_resize_handles();
    requestAnimationFrame(() => this.update_handle_position());
    this.date_changed();
    this.compute_duration();
  ...
  update_handle_position() {
    // if (this.invalid || this.gantt.options.readonly) return;
    // const bar = this.$bar;
    console.log("✅ Custom Bar.js loaded for task:", task.name);
     console.log(`[bar.js] update_handle_position for:`, this.task?.id);
    if (this.invalid || this.gantt.options.readonly) return;
    const bar = this.$bar;
    if (!bar) return;

    console.log("Left handle?", this.handle_group?.querySelector(".handle.left"));
console.log("Right handle?", this.handle_group?.querySelector(".handle.right"));
    const handleLeft = this.handle_group?.querySelector(".handle.left");
    if (handleLeft) {
      handleLeft.setAttribute("x", bar.getX());
    } else {
      console.warn("Left handle missing:", this.task.id);
    }

    const handleRight = this.handle_group?.querySelector(".handle.right");
    if (handleRight) {
      handleRight.setAttribute("x", bar.getEndX());
    } else {
      console.warn("Right handle missing:", this.task.id);
    }

    const handleProgress = this.group?.querySelector(".handle.progress");
    if (handleProgress && this.$bar_progress) {
      handleProgress.setAttribute("cx", this.$bar_progress.getEndX());
    } else {
      console.warn("Progress handle or progress bar missing:", this.task.id);
    }
